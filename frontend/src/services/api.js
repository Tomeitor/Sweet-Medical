...
export async function rescheduleAppointmentByPatient(id, fechaHora, motivo) {
  const response = await api.patch(`/turnos/${id}/cambio`, {
    fechaHora,
    motivo,
  });
  return response.data;
...
export async function proposeAppointmentChange(id, fechaHora, motivo) {
  const response = await api.patch(`/turnos/${id}/proponer-cambio`, {
    fechaHora,
    motivo,
  });
  return response.data;
...
export async function respondToAppointmentProposal(id, notificacionId, accion) {
  const response = await api.patch(`/turnos/${id}/propuesta/responder`, {
    notificacionId,
    accion,
  });
  return response.data;
...
export async function cancelAppointmentByDoctor(id, motivo) {
  const response = await api.patch(`/turnos/${id}/cancelacion-medico`, {
    motivo,
  });
  return response.data;
}
...
export async function fetchNotifications(usuarioId, leida) {
  const response = await api.get(
    `/notificaciones/usuarios/${usuarioId}/notificaciones`,
    {
...
export async function markNotificationAsRead(usuarioId, notificacionId) {
  const response = await api.patch(
    `/notificaciones/usuarios/${usuarioId}/notificaciones/${notificacionId}`,
    {
      leida: true,
    },
  );
